...
formatButton s =
    label [ Html.Attributes.style [("padding", "20px")] ]
          [ input [ Html.Attributes.type_ "radio"
...
formatDiv m =
    let fmts = m.knownFormats
        btns = List.map formatButton (Maybe.withDefault ["markdown"] fmts)
        warning = if isNothing fmts
                  then div [] [text "Cannot fetch list of supported formats. Defaulting to 'markdown'"]
                  else div [] []
        s = [warning] ++ btns
    in div [] [ h2 [] [text "Select Format to Preview"]
              , div [] s
              ]
...
inputBox : Html Msg
inputBox = div [] [ h2 [] [text "Input"]
                  , textarea [onInput UpdateContent] []
                  ]

outputBox : Model -> Html Msg
outputBox m =
    let innerHTML = Maybe.withDefault "" m.outputData in
    div [] [ div [] [h2 [] [text "Preview"]]
           , iframe [srcdoc innerHTML] []
           ]

init : (Model, Cmd Msg)
init = (mkModel, fetchKnownFormats)
